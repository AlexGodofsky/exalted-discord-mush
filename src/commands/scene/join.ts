...
export async function handler(argv: yargs.Arguments) {
	try {
		await join(argv.context, argv.message, argv.scene_id, argv.char_name);
		argv.resolve();
	} catch (error) {
...
export async function join(context: Context, message: Message, scene_id: number, char_name: string) {
